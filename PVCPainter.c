...
PBitmap pvcLoadBitmap(char * filename)
{
  ...
  if (fd < 0)
  {
	...
	int s = l * bmp.height;
	printf(1, "load bitmap l%d s%d c%d width %d height %d\n",l,s, c,bmp.width,bmp.height);
	
	

	//jump to the real start of the image, fixed by 7ziban group
	int wastelen = fileHeader.bfOffBits-sizeof(fileHeader)-sizeof(infoHeader);
	printf(1,"waste len %d", wastelen);
	unsigned char * waste = (unsigned char *)malloc(sizeof(unsigned char) * wastelen);
	read(fd, (char *)waste, wastelen);

	unsigned char * data = (unsigned char *)malloc(sizeof(unsigned char) * fileHeader.bfSize);
	read(fd, (char *)data, sizeof(unsigned char) * fileHeader.bfSize);
	...
	for (int j = 0; j < bmp.height; ++j)
	{
		...
		{
			bmp.data[off + i].r = (int)data[dataoffset +bit * i + bit - 1];
			bmp.data[off + i].g = (int)data[dataoffset +bit * i + bit - 2];
			bmp.data[off + i].b = (int)data[dataoffset +bit * i + bit - 3];
		}
	}
	close(fd);
	return bmp;
...
void pvcTransparentBit(PHdc buf, int x, int y, PHdc src, int sx, int sy, int w, int h, PColor tspt)
{
	...
	for (i = 0; i < h; ++i)
	{
		...
		for (j = 0; j < w; ++j)
		{
			...
			PColor c = src->content[off_src + j];
			if (isValidColor(c) )//&& (c.r != tspt.r || c.g != tspt.g || c.b != tspt.b)) // remove the transparent setting, fixed by hsd
				buf->content[off_buf + j] = c;
		}
	}
	/*int i, j;
