...
void fromOtherAppCallFileManager(){
  PHwnd hwnd = pvcCreateWindow("FileManager", 0, 100, 100, 400, 400);
  pvcWndExec(hwnd, wndProc);
}
...
void ListItem(char *path)
{
  ...
  case T_FILE:
  {
    ...
    else if(l > 3 && (( temp[l-3]=='b' && temp[l-2]=='m' && temp[l-1]=='p') ||
                      ( temp[l-3]=='j' && temp[l-2]=='p' && temp[l-1]=='g') ||
                      ( temp[l-3]=='p' && temp[l-2]=='n' && temp[l-1]=='g') ))
                    {
                      printf(1,"detect it is a picture");
                      char* cmds[] = { temp, 0 };
                      runAppWithCmds("PhotoViewer",cmds);
                    } 
	else if (l != 0)
	{
		char* cmds[] = { temp, 0 };
